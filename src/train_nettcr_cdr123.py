...
def train(args):
    ...
    if args.chain == "ab":
        ...
        net = NetTCR_CDR123_singlechain()
    net.to(device)
    criterion = nn.BCELoss()
    optimizer = optim.Adam(net.parameters(), lr=args.learning_rate)

    early_stopping = utils.EarlyStopping(patience=20, 
                                   path=args.outdir+'/trained_model_cdr123_'+args.chain+'.pt',
                                   print_count=False)
    ...
    for epoch in range(args.epochs):
        ...
        batch_loss = 0
        net.train()
        for pep_train, a1_train, a2_train, a3_train, \
            b1_train, b2_train, b3_train, y_train in train_loader:

            net.zero_grad()
            if args.chain == "ab":
                output = net(pep_train, a1_train, a2_train, a3_train, b1_train, b2_train, b3_train)
            elif args.chain == "a":
                output = net(pep_train, a1_train, a2_train, a3_train)
            elif args.chain == "b":
                output = net(pep_train, b1_train, b2_train, b3_train)
        ...
        val_batch_loss = 0
        net.eval()
        for pep_valid, a1_valid, a2_valid, a3_valid, \
            b1_valid, b2_valid, b3_valid, y_valid in valid_loader:

            if args.chain == "ab":
                pred = net(pep_valid, a1_valid, a2_valid, a3_valid, b1_valid, b2_valid, b3_valid)
            elif args.chain == "a":
                pred = net(pep_valid, a1_valid, a2_valid, a3_valid)
            elif args.chain == "b":
                pred = net(pep_valid, b1_valid, b2_valid, b3_valid)
